...
# small utils
from pipeline.ingestion.grobid_ingest import _sanitize_filename  # keep filename policy consistent with grobid ingest

# embedding/upsert delegation (reuse embed_runner behavior)
from pipeline.runner.embed_runner import embed_and_upsert  # streamlined embed runner
...
def parse_teis_to_chunks(
    ...
    summary = {"n_input_files": len(tei_files), "n_written": 0, "n_skipped": 0, "n_failures": 0, "files": [], "errors": []}
    ...
    for tei_path in tei_files:
        ...
        try:
            ...
            # idempotency check
            tei_parsed_ix = chunks_dir + '/papers'
            done = _read_done_marker(tei_parsed_ix, paper_id)
            ...
            # WRITE: pass model instances directly (chunks_fs expects models and calls model.dict())
            ...
            _write_done_marker(tei_parsed_ix, paper_id, done_meta)
            logger.info("parse: wrote %d chunks for %s", n_chunks, paper_id)
            summary["n_written"] += 1
            summary["files"].append({"tei": str(tei_path), "paper_id": paper_id, "status": "written", "n_chunks": n_chunks})
...
def main(
    ...
    parse_summary = parse_teis_to_chunks(tei_dir, chunks_dir, min_len=min_len, dry_run=dry_run, force=force)
